    ...
    <ion-grid>
        ...
        <ion-row class="calendar-row" *ngFor="let week of weekArray;let i = index">
            ...
            *ngFor="let day of week;let j = index"
            [ngClass]="[day.isThisMonth?'this-month':'not-this-month',day.isToday?'today':'',day.isSelect?'select':'',''+day.status]">
                {{day.date}}
...
export class Calendar {
  ...
  isInEvents(year, month, date) {
    ...
    for (; i < len; i++) {
      if (
        this.events[i].year == year &&
        this.events[i].month == month &&
  ...
  createMonth(year: number, month: number) {
    ...
    if (firstDay !== 7) {
      // Sunday doesn't need to be shown for the previous month
      ...
      for (let i = 0; i < firstDay; i++) {
        if (month === 0) {
          this.dateArray.push({
            ...
            isToday: false,
            isSelect: false,
            hasEvent: this.isInEvents(year, 11, lastMonthStart + i)
              ? true
              : false
          });
        } else {
          this.dateArray.push({
            ...
            isToday: false,
            isSelect: false,
            hasEvent: this.isInEvents(year, month - 1, lastMonthStart + i)
              ? true
              : false
          });
        }
